...
- (id)initWithMenuViewController:(UIViewController *)menu contentViewController:(UIViewController *)content {
  ...
  _percentageOfScreenWidthUsedByMenu = 86.25;
...
- (void)pan:(UIPanGestureRecognizer *)pan {
  ...
  CGFloat maximumX = [self menuWidth];
...
- (CGRect)contentViewFrameForStaging {
  CGRect frame = self.contentView.frame;
  frame.origin.x = CGRectGetMaxX(self.view.frame) + [UIScreen mainScreen].bounds.size.width - [self menuWidth];
...
- (CGFloat)menuWidth {
  return [UIScreen mainScreen].bounds.size.width * (self.percentageOfScreenWidthUsedByMenu / 100.0f);
}
